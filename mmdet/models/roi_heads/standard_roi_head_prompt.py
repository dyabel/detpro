...
class StandardRoIHeadPrompt(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 kd_weight = 256,
                 prompt_path='lvis_text_embedding.pt'
                 ):
        super(StandardRoIHeadPrompt, self).__init__(bbox_roi_extractor=bbox_roi_extractor,
                                              bbox_head=bbox_head,
        ...
        print(prompt_path)
        self.image_text_embedding =  torch.load(prompt_path).to(device).squeeze()[:1203,:]
        self.image_text_embedding = F.normalize(self.image_text_embedding,dim=-1)
        print(self.image_text_embedding.shape)
        save_path = dataset + '_text_embedding.pt'
        time_start = time.time()
        if osp.exists(save_path):
            self.text_features_for_classes = torch.load(save_path).to(device).squeeze()[:1203,:]
        else:
            self.clip_model, self.preprocess = clip.load('ViT-B/32', device)
            self.clip_model.eval()
            for child in self.clip_model.children():
        ...
        if self.load_feature:
            ...
            for i in range(len(img_metas)):
                ...
                try:
                    ...
                    # clip_image_features = self.clip_image_forward((img_metas[i],), bboxes,(num_proposals_per_img[i],))
                    # clip_image_features15 = self.clip_image_forward((img_metas[i],), bboxes15,(num_proposals_per_img[i],))
                    clip_image_features = self.img2pil2feat(img_no_normalize[i], bboxes[:,1:])
    ...
    def simple_test_bboxes(self,
        ...
        bg_class_embedding = torch.nn.functional.normalize(bg_class_embedding,p=2,dim=1)
        text_features = torch.cat([self.text_features_for_classes,bg_class_embedding],dim=0)
        image_text_features = torch.cat([self.image_text_embedding,bg_class_embedding],dim=0)
        #-----------------------------------------------------
        # """
        cls_score_text = region_embeddings@text_features.T
        ...
        if self.ensemble and not self.use_clip_inference:
            ...
            region_embeddings_image = self.projection_for_image(region_embeddings_image)
            region_embeddings_image = torch.nn.functional.normalize(region_embeddings_image,p=2,dim=1)
            cls_score_image = region_embeddings_image@image_text_features.T
